...
static int bba_copy_dma(uint8 * dst, uint32 s, int len)
{
  ...
  if (len > DMA_THRESHOLD) {
    ...
    /* This way all will be nicely 32 bytes aligned (assuming that dst and src have same
       alignement initially and that we don't care about the beginning of dst buffer) */
    add = ((uint32)src) & 31;
    len += add;
    src -= add;
    dst -= add;
...
#ifndef USE_P2_AREA
    /* used to be a call to dcache_inval_range, but for some strange reasons, 
       I need to make a full flush now ... */
...
static int rx_enq(int ring_offset, int pkt_size) {
  ...
  if (eth_rx_callback) {
    ...
    if ( rxin != rxout && 
	 (( (rx_pkt[rxout].rxbuff - (rxbuff+32+BEFORE)) - rxbuff_pos ) & (RXBSZ-1)) < 
	 pkt_size + 2048) {
